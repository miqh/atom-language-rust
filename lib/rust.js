...
class Rust {
  ...
  loadStylesFile() {
    let package = atom.packages.getLoadedPackage(PACKAGE_NAME);
    if (!package) {
    ...
    this.stylesPath = [package.path, 'styles', 'rust.less'].join(path.sep);
    let styles;
    try {
  ...
  readDefaultColors() {
    let styles =
      atom.themes.lessCache.cssForFile(null, this.styles);
